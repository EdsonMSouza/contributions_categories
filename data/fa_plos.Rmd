---
title: "Factorial Analysis: PLoS Medicine"
author:
  - "Edson Souza"
  - "Wonder Alexandre"
  - "Jose Storopoli"
...
df <- read_excel("plos.xlsx")
df_factor <- df %>% select(starts_with("au_c"))
```

### Features variation
```{r ggplot}
df %>% select(au_c01:au_c14) %>% pivot_longer(everything()) %>% ggplot(., aes(name, value)) + geom_boxplot(outlier.colour = "red", outlier.shape = 1)
```
...
shapiro_df <- function(df) {
  colunas <- names(df)
  p_vals <- purrr::map_dbl(colunas, ~{shapiro.test(sample({{ df }}[[{{ .x }}]], 4127))$p.value})
  if (any(p_vals < 0.05)) {
...
### Barlett's test
```{r Barlett}
df %>% select(au_c01:au_c10) %>% bartlett.test(.)
```

### KMO - Kaiser-Meyer-Olkin factor adequacy
```{r kmo}
df %>% select(au_c01:au_c10) %>% KMO(.)
```
