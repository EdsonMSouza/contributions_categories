...
df <- read_excel("jama.xlsx")
df_factor <- df %>% select(starts_with("au_c"))
```

### Features variation
```{r ggplot}
df %>% select(au_c01:au_c10) %>% pivot_longer(everything()) %>% ggplot(., aes(name, value)) + geom_boxplot(outlier.colour = "red", outlier.shape = 1)
```
...
### 2 factors
```{r psych-2}
res2 <- fa(df_factor, rotate = "varimax", fm = "pa", nfactors = 2, cor="mixed")
